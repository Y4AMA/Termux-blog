#  Neko Coding - Termux Blog

![Termux Banner](https://raw.githubusercontent.com/termux/termux-app/master/app/src/main/res/mipmap-xxxhdpi/ic_launcher.png)

Selamat datang di **Neko Coding** - blog pribadi yang membahas seputar Termux, programming, dan tips trik kucing-kucingan ala developer 🐾

## 📱 Tentang Blog Ini

Blog ini dibuat untuk mendokumentasikan perjalanan gue belajar Termux dan hal-hal teknis lainnya. Dari yang beginner sampai yang bikin pusing, semua bakal dicatet di sini.

**Akses blog:** [neko-coding.zaptor.org](http://neko-coding.zaptor.org)

## 📋 Daftar Konten

### 🔰 Tutorial Dasar Termux
- [Installasi Termux di Android](link-ke-post)
- [Perintah Dasar Termux untuk Pemula](link-ke-post)
- [Setup Storage dan Permission](link-ke-post)
- [Kustomisasi tampilan Termux](link-ke-post)

### ⚙️ Tools & Module
- [Install Module Python Lengkap](link-ke-post)
- [Tips dan Trik Paket di Termux](link-ke-post)
- [Rekomendasi Tools Hacking Etis](link-ke-post)
- [Belajar Git di Termux](link-ke-post)

### 🚀 Project Keren
- [Membuat Bot Telegram Sederhana](link-ke-post)
- [Setup Web Server Lokal di HP](link-ke-post)
- [Termux sebagai Server Pribadi](link-ke-post)

### 🐧 Linux untuk Android
- [Bedanya Termux dengan Linux biasa](link-ke-post)
- [Install Distro Linux di Termux (proot)](link-ke-post)
- [Tips Optimasi Termux biar nggak lemot](link-ke-post)

## 📸 Screenshots
